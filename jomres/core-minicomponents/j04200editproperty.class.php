...
class j04200editproperty
	...
	function j04200editproperty( $componentArgs )
		...
		if ( isset( $propertyUid ) && $propertyUid != "" )
			{
			$query        = "SELECT propertys_uid,property_name,property_street,property_town,property_region,property_regionpromo,property_booking_configuration,property_country,property_postcode,
			...
			foreach ( $propertyList as $property )
				...
				if ( is_numeric( $property->property_regionpromo ) ) $propertyRegionPromo = jr_gettext( '_JOMRES_CUSTOMTEXT_REGIONNAMES_' . $selectedCountry . "_" . $property->property_regionpromo, $property->property_regionpromo, false, false );
				else
				$propertyRegionPromo = $property->property_regionpromo;

				}
				$booking_configuation = "";
				$booking_configuation = $property->property_booking_configuration;
			}
		...
		if ( $jrConfig[ 'limit_property_country' ] == "0" )
			...
			$output[ 'REGIONPROMODROPDOWN' ]    = setupRegions( $selectedCountry, $propertyRegionPromo );
			}
		else
			{
			...
			$output[ 'REGIONPROMODROPDOWN' ]    = setupRegions( $jrConfig[ 'limit_property_country_country' ], $propertyRegionPromo, true, 'region_promo' );
			}

			if($booking_configuation =="numbered_site"){
				$output['BOOKINGCONFIGURATION'] = "<select id=\"booking_configuration\" name=\"booking_configuration\" class=\"input-medium\">
					<option value=\"site_type\">Book site type only</option>
					<option selected=\"selected\" value=\"numbered_site\">Book specific numbered site</option>
				</select>";
			}
			else {
				$output['BOOKINGCONFIGURATION'] = "<select id=\"booking_configuration\" name=\"booking_configuration\" class=\"input-medium\">
					<option selected=\"selected\" value=\"site_type\">Book site type only</option>
					<option value=\"numbered_site\">Book specific numbered site</option>
				</select>";
			}
		...
		$output[ 'HREGIONPROMO' ]    = jr_gettext( '_JOMRES_COM_MR_VRCT_PROPERTY_HEADER_REGION', _JOMRES_COM_MR_VRCT_PROPERTY_HEADER_REGION );
