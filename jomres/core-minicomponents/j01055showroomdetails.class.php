...
//print_r($all);
		...
		if ( count( $roomList ) > 0 )
			...
			foreach ( $roomList as $room )
                ...
                $headersList['PRO'] = $property_uid;

                $mapType                  = jomresGetParam( $_REQUEST, 'map', '' );
                $headersList['MAPTEXT'] = "Download the campground map";

                if(isset($mapType) && $mapType == "mud"){
                    $headersList['MAPTEXT'] = "Campground mud map";
                }
...
//$pro['PRO'] = $property_uid;    
//$tmpl->addRows('property',$pro);
