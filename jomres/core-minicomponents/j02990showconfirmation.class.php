...
class j02990showconfirmation
	...
	function j02990showconfirmation()
		...
		$pos = strpos($booking_parts[ 'PROPERTYNAME' ],"Camp");
		if( $pos === false)
			$booking_parts[ 'PROPERTYNAME' ] = $booking_parts[ 'PROPERTYNAME' ] . " Campground";

		$booking_parts[ 'PROPERTY' ] = $bookingDeets[ 'property_uid' ];
		$booking_parts[ 'PROPERTY_AIRPORTS' ] = $current_property_details->property_airports;
		...
		$query         = "SELECT room_number,room_name,room_classes_uid,room_features_uid FROM #__jomres_rooms WHERE $gor ORDER BY room_classes_uid";
		$roomList      = doSelectSql( $query );
		$roomNumber    = "";
		$room_name     = "";
		$prevroomclass = 0;

// Nitin changed for Purnululu and walardi - kununarra
...
// Nitin modified for Purunulu and walardi - kununarra
	...
	//		$room_info[ ] = array ( "ROOM_NAME" => $room_name, "ROOM_NUMBER" => $room->room_number, "ROOM_FEATURE" => $feature  ,"HROOM_NAME" => jr_gettext( "_JOMRES_COM_MR_EB_ROOM_NAME" ), "HROOM_NUMBER" => jr_gettext( "_JOMRES_COM_MR_EB_ROOM_NUMBER" ) );
...
//        die();
        ...
        $booking_parts[ 'TERMS' ]  = makePopupLink("http://" .$host. "/terms-and-conditions", jr_gettext( '_JOMRES_COM_MR_VRCT_PROPERTY_HEADER_POLICIESDISCLAIMERS', _JOMRES_COM_MR_VRCT_PROPERTY_HEADER_POLICIESDISCLAIMERS ), true, 750, 500 );
		...
		if ( $paypal_settings->paypalConfigOptions[ 'override' ] != "1" )
			...
			if ( !$userIsManager && $mrConfig[ 'useOnlinepayment' ] != "0" )
                ...
                // Nitin tested for payment gateway information
               // print_r($query);
                //print_r($gatewayDeets );
