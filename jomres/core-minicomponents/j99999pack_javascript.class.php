...
class j99999pack_javascript
	...
	function j99999pack_javascript()
		...
		return;
		define( "JOMRES_JS", 1 );
		if ( jomres_cmsspecific_areweinadminarea() ) return;
	...
	function check_watch_file( $type = "js", $identifier, $watched_file )
		...
		if ( $type == "js_1" || $type == "js_2" ) $abs_path = get_showtime( 'js_cache_path' );
		else
		...
		if ( $d )
			...
			if ( count( $docs ) > 0 )
				...
				foreach ( $docs as $f )
					...
					if ( $seconds_timediff > $timeout ) unlink( get_showtime( 'js_cache_path' ) . "/" . $f );
					}
				}
			}
		}
