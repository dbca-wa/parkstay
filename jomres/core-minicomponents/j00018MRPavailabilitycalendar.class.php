...
class  j00018MRPavailabilitycalendar
	...
	function  j00018MRPavailabilitycalendar( $componentArgs = null )
		...
		if ( isset( $mrConfig[ 'CalendarMonthsToShow' ] ) ) $showFullYear = $mrConfig[ 'CalendarMonthsToShow' ];
		...
		$this->colour_empty        = '#529e54';
		$this->colour_quarter      = '#b6d100';
		$this->colour_half         = '#f1c421';
		$this->colour_threequarter = '#f5a11a';
		$this->colour_full         = '#d03e25';
		...
		if ( $mrConfig[ 'showAvailabilityCalendar' ] == "1" )
			{
			$currentYear = date( "Y" );
			if ( $mrConfig[ 'calstartfrombeginningofyear' ] == "1" ) $currentMonth = 1;
			else
			$currentMonth = date( "m" );
			$counter         = 1;
			...
			<center>
			';
			if ( using_bootstrap() ) $this->retVals .= '<div class="well" style="display:none; margin-bottom: 10px;">';
			else
	...
	function makecal( $stmonth, $styear, $roomUid, $property_uid )
		...
		if ( $task == "remoteavailability" ) $target = 'target="_blank"';
		else
		$target = '';
		if ( $tar == 0 ) $target = '';
		if ( $show_links == 0 ) $this->showlinks = false;
		###################################
		...
		if ( $jrConfig[ 'calendarstartofweekday' ] == "1" ) $startdate = $currdate = mktime( 0, 0, 0, $stmonth, 1 - date( "w", mktime( 0, 0, 0, $stmonth, 1, $styear ) ), $styear );
		else
		...
		foreach ( $roomList as $cont )
			...
			if ( !array_key_exists( $date, $tmpdatesArray ) ) $tmpdatesArray[ $date ] = 1;
			else
				{
		...
		foreach ( $tmpdatesArray as $key => $d )
			...
			if ( $d == $this->rooms_full ) $bgcolor = $this->colour_full;
			if ( $d <= $this->rooms_full - 1 && $d >= $this->rooms_threequarter ) $bgcolor = $this->colour_threequarter;
			if ( $d < $this->rooms_threequarter && $d >= $this->rooms_half ) $bgcolor = $this->colour_half;
			if ( $d < $this->rooms_half && $d >= $this->rooms_empty + 1 ) $bgcolor = $this->colour_quarter;
			if ( $d == $this->rooms_empty ) $bgcolor = $this->colour_empty;
		...
		while ( $currdate < $enddate )
			...
			for ( $c = 0; $c < 7; $c++ )
				...
				if ( $currdate < $stdate ) $bgcolor = $outbgcolor;

				if ( $mrConfig[ 'limitAdvanceBookingsYesNo' ] == "1" && ( $unixCurrDate >= $unixLatestDate ) ) $bgcolor = $pastcolour;
				if ( $unixCurrDate < $unixTodaysDate ) $bgcolor = $pastcolour;
				...
				if ( $mrConfig[ 'showOnlyAvailabilityCalendar' ] == "1" ) $this->showlinks = true;

				if ( !$showOutMonthDates && $bgcolor == $outbgcolor ) $this->retVals .= "&nbsp;";
				else
					{
					if ( $bgcolor != $outmonthface && ( $bgcolor != $pastcolour ) )
						...
						if ( !$noshowroom && $bgcolor != $outbgcolor && $mrConfig[ 'visitorscanbookonline' ] && $bgcolor != $this->colour_full && $this->showlinks )
							...
							if ( $mrConfig[ 'fixedArrivalDateYesNo' ] )
								...
								if ( $mrConfig[ 'fixedArrivalDay' ] != date( "w", $currdate ) ) $validDayOfWeek = false;
								}
							...
							if ( $validDayOfWeek )
								...
								*/
								if ( $jrConfig[ 'useSSLinBookingform' ] == "1" ) $link = jomresURL( $link, 1 );
								else
								$link = jomresURL( $link );
