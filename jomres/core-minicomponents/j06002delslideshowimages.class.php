			...
			if ( file_exists( $mrp . $imageName ) )
				...
				if ( is_writable( $mrp . $imageName ) )
					...
					if ( !unlink( $mrp . $imageName ) )
						...
						if ( file_exists( $mrp . $filename_med ) ) unlink( $mrp . $filename_med );
						if ( file_exists( $mrp . $filename_small ) ) unlink( $mrp . $filename_small );

						$joomla_dir = $mrp . 'joomla' . JRDS;
						if ( file_exists( $joomla_dir . "large" . JRDS . $filename_large ) ) unlink( $joomla_dir . "large" . JRDS . $filename_large );
						if ( file_exists( $joomla_dir . "medium" . JRDS . $filename_med ) ) unlink( $joomla_dir . "medium" . JRDS . $filename_med );
						if ( file_exists( $joomla_dir . "small" . JRDS . $filename_small ) ) unlink( $joomla_dir . "small" . JRDS . $filename_small );
