...
class j02300regprop1 {
	...
	function j02300regprop1()
		...
		if (isset($_REQUEST['xyz']) && !empty($_REQUEST['xyz']))
			$selectedCountry	= jomresGetParam( $_REQUEST, 'xyz', "" );
		else
			{
		...
		if ($jrConfig['limit_property_country'] == "0")
			{
			$output['REGIONDROPDOWN']=setupRegions($selectedCountry,$propertyRegion);
			$output['COUNTRIESDROPDOWN']=createCountriesDropdown($selectedCountry,"xyz");
			}
		else
			{
		...
		$output['_JOMRES_REGISTRATION_STEP_1_OF_2']=jr_gettext('_JOMRES_REGISTRATION_STEP_1_OF_2',_JOMRES_REGISTRATION_STEP_1_OF_2,false);
