...
class j06001ajax_dashboard_save {
	function j06001ajax_dashboard_save($componentArgs)
		{
		...
		$gor = genericOr($this->dates_array,'date',false);
		...
		foreach ($this->rooms_array as $room_uid)
			{
			$query="SELECT contract_uid FROM #__jomres_room_bookings WHERE property_uid = ".(int)$this->property_uid." AND `room_uid`=".$room_uid." AND  ".$gor."";
	...
	function create_new_contract()
		...
		$last = count($this->dates_array)-1;
		$departureDate=$this->dates_array[$last];
