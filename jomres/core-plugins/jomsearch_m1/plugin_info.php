...
class plugin_info_jomsearch_m1
	...
	function plugin_info_jomsearch_m1()
		...
		$this->data=array(
			...
			'change_log'=>' v1.1 Added pricerange option to enable searching by price ranges from Jomres 3.2.1. 1.2 Removed the memory_limit setting code as this is handled in jomres.php 1.3 Added code for showing country/region/town jquery combo dropdowns. mod_jomsearch_m1 is optimised to be shown in a banner/syndication template area, the other three search modules srch.html template files are left with no layout so that you can organise them as you want. 1.4 updated to make compatible with Jomres v4.2  1.6 Updated to work in tandem with v4.6.0 offering search by guest numbers and number of stars.  v2 modified to work in Joomla 1.6 2.1 Updated module to bring it into line with v5.1. 2.2 Added a dependencies check file to ensure that users are advised that alt_init needs to be installed before they install this plugin. 2.3 Added a check to see if _JOMRES_INITCHECK has already been defined. 2.4 added module class suffix. 2.5 layout tweaks to template file. 2.6 Modified the form url, to ensure that the language parameter is sent with queries. 2.7 minor code tidyup. v2.8 Changed the construction of the form url.',
