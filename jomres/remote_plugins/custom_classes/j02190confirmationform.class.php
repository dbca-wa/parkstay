...
class j02190confirmationform
	...
	function j02190confirmationform()
		...
		$contract_total = 0;
		$room_class="";
		$query           = "SELECT arrival,departure,guest_uid,rate_rules,contract_total,deposit_required,tag,property_uid,invoice_uid FROM #__jomres_contracts WHERE contract_uid = '" . (int) $contract_uid . "' AND property_uid = '" . (int) $defaultProperty . "'";
		...
		foreach ( $contractData as $contract )
			...
			$invoice_id   	  = $contract->invoice_uid;
			}
		...
		//Additional output items from guest conf email
		$propertyID = $propertyUid;

		$query         = "SELECT property_name,property_town,property_description,property_checkin_times,property_tel,property_email,property_airports,property_othertransport,property_policies_disclaimers,property_booking_configuration FROM #__jomres_propertys WHERE propertys_uid = '" . $propertyID . "' LIMIT 1";
...
//		$output['SP_VAT']=jr_gettext('_JOMRES_COM_CONFIRMATION_RESERVATION_VAT',_JOMRES_COM_CONFIRMATION_RESERVATION_VAT);
		...
		//Additional output items from guest conf email
		$output[ 'LINK' ]           = JOMRES_SITEPAGE_URL_NOSEF . "&task=viewproperty&property_uid=" . $propertyID;
		$output[ 'LINKTOPROPERTY' ] = "<a style=\"color: blue;\" href=\"" . JOMRES_SITEPAGE_URL_NOSEF . "&task=viewproperty&property_uid=" . $propertyID . "\">" . jr_gettext( '_JOMRES_COM_MR_VRCT_PROPERTY_HEADER_WEBSITE', _JOMRES_COM_MR_VRCT_PROPERTY_HEADER_WEBSITE, false, false ) . "</a>";
		...
		$query         = "SELECT firstname,surname,email,tel_landline,tel_mobile FROM #__jomres_guests WHERE guests_uid = '" . (int) $guestUid . "'";
...
// Nitin Start
			...
			//	$result = jomresMailer( $output[ 'PROP_EMAIL' ], $output[ 'PROP_NAME' ], $output[ 'PROP_EMAIL' ], $subject, $text, $mode = 1, $attachments ); // Let's send this to the property too, so that they have a seperate record
				...
				//$result=jomresMailer( "campgrounds@dpaw.wa.gov.au", $propertyName, $useremail, $subject, $text,$mode=1);
				//$result=jomresMailer( "campgrounds@dpaw.wa.gov.au", $propertyName, $useremail, $subject, $text,$mode=1);
				$result=jomresMailer( "campgrounds@dpaw.wa.gov.au", $propertyName, $useremail, $subject, $text,$mode=1);// Let's send this to the property too, so that they have a seperate record
