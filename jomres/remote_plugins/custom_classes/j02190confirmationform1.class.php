...
// Nitin Start
			...
			//	$result = jomresMailer( $output[ 'PROP_EMAIL' ], $output[ 'PROP_NAME' ], $output[ 'PROP_EMAIL' ], $subject, $text, $mode = 1, $attachments ); // Let's send this to the property too, so that they have a seperate record
				...
				$result=jomresMailer( "webcontent@dec.wa.gov.au", $output['PROP_NAME'], $useremail, $subject, $text,$mode=1);
				$result=jomresMailer("webcontent@dec.wa.gov.au",$output['PROP_NAME'], $output['PROP_EMAIL'], $subject, $text,$mode=1);// Let's send this to the property too, so that they have a seperate record
