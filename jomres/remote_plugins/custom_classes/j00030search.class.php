...
#print_r($_REQUEST);
		...
		if ( !empty( $_REQUEST[ 'region_promo' ] ) )
			{
			if ( $_REQUEST[ 'region_promo' ] == $searchAll ) $sch->filter[ 'region' ] = "%";
			else
				{
				$sch->filter[ 'region' ] = jomresGetParam( $_REQUEST, 'region_promo', "" );
				$sch->filter[ 'region' ] = str_replace( $unwanted, "", $sch->filter[ 'region' ] );
				$region_name             = find_region_name( $sch->filter[ 'region' ] );
				$sch->filter[ 'region_promo' ] = jomresGetParam( $_REQUEST, 'region_promo', "" );
				$sch->filter[ 'region_promo' ] = str_replace( $unwanted, "", $sch->filter[ 'region_promo' ] );
				$regionpromo_name             = find_region_name( $sch->filter[ 'region' ] );
				$metaTitle .= " " . htmlspecialchars_decode( $region_name, ENT_QUOTES );
				}
			}
        ...
        if(isset($_REQUEST['region_promo']) && !empty( $_REQUEST[ 'region_promo' ])){
            $availability = true;
        }

		if ( !empty( $_REQUEST[ 'arrivalDate' ] ) && (in_array( 'availability', $sch->searchOptions ) || $availability) )
			{
			$sch->filter[ 'arrival' ]   = $_REQUEST[ 'arrivalDate' ];
			$sch->filter[ 'departure' ] = $_REQUEST[ 'departureDate' ];
			$sch->filter[ 'arrival' ]   = str_replace( $unwanted, "", $sch->filter[ 'arrival' ] );
			$sch->filter[ 'departure' ] = str_replace( $unwanted, "", $sch->filter[ 'departure' ] );
                //var_dump($sch->filter);exit;
			}
		...
		if ( in_array( "availability", $searchOptions ) && $showSearchOptions && get_showtime( 'task' ) != "bookaroom" )
			...
			$output[ 'JOMRES_SEARCH_AVL_INFO' ] = jr_gettext( '_JOMRES_SEARCH_AVL_INFO', _JOMRES_SEARCH_AVL_INFO, false );
                ...
                if($calledByModule == "mod_jomsearch_m3"){
        ...
        if($calledByModule == "mod_jomsearch_m3"){
            $output[ 'SUBMITURL' ] = "/camp-finder/index.php?lang=en";
        }
		...
		if ( in_array( "region", $searchOptions ) && $showSearchOptions && !in_array( "selectcombo", $searchOptions ) )
			{
                ...
                $output['hbookable'] = "Bookings cannot be made for most campgrounds.";
//            $output['bookable']='<input type="checkbox" name="nodates" value="1" id="JIfzMQrkwz" onClick="jomresJquery(\'#oSPtVgIgCODYpjH_XXX\').datepicker( \'isDisabled\' )?jomresJquery(\'#oSPtVgIgCODYpjH_XXX\').datepicker( \'enable\' ):jomresJquery(\'#oSPtVgIgCODYpjH_XXX\').datepicker( \'disable\' );jomresJquery(\'#oSPtVgIgCODYpjH\').datepicker( \'isDisabled\' )?jomresJquery(\'#oSPtVgIgCODYpjH\').datepicker( \'enable\' ):jomresJquery(\'#oSPtVgIgCODYpjH\').datepicker( \'disable\' );" />';
            ...
            if ( true )
            {
                ...
                if ( $searchOutput[ 'town' ] == "dropdown" )
                {
                    ...
                    // Nitin Start
                    // Customised search to meet custom search feature
                    //$output[ 'town' ] = jomresHTML::selectList( $townArray, 'town', 'size="1" ', 'value', 'text', $selectOption );
                    $output[ 'town' ] = '<input class="inputbox" style="width:90%;height:20px;margin-right:10px;" placeholder="Park, nearby town or other place name" type="text" name="town"  onfocus="if (this.value ==\'' . $output[ 'JOMRES_SEARCH_TOWN_LABEL' ] . '\') {this.value = \'\'}" />';
                    // End
                    $showButton       = true;
                }
                else
				...
				if ( $searchOutput[ 'region' ] == "dropdown" )
					...
					foreach ( $sch->prep[ 'region' ] as $region )
						...
						$region_id      = find_region_id( jomres_cmsspecific_stringURLSafe( $t ) );
                            if($region_id == 3929 || $region_id == 3930 || $region_id == 3931 || $region_id == 3932){
                            }
                            else{
...
//            $output['bookable']='<input type="checkbox" name="nodates" value="1" id="JIfzMQrkwz" onClick="jomresJquery(\'#oSPtVgIgCODYpjH_XXX\').datepicker( \'isDisabled\' )?jomresJquery(\'#oSPtVgIgCODYpjH_XXX\').datepicker( \'enable\' ):jomresJquery(\'#oSPtVgIgCODYpjH_XXX\').datepicker( \'disable\' );jomresJquery(\'#oSPtVgIgCODYpjH\').datepicker( \'isDisabled\' )?jomresJquery(\'#oSPtVgIgCODYpjH\').datepicker( \'enable\' ):jomresJquery(\'#oSPtVgIgCODYpjH\').datepicker( \'disable\' );" />';
		...
		// Customised search to meet custom search feature				
					//$output[ 'town' ] = jomresHTML::selectList( $townArray, 'town', 'size="1" ', 'value', 'text', $selectOption );
					$output[ 'town' ] = '<input class="inputbox" style="width:90%;height:40px;margin-right:10px;font-size:14px;" placeholder="Park, nearby town or other place name" type="text" name="town"  onfocus="if (this.value ==\'' . $output[ 'JOMRES_SEARCH_TOWN_LABEL' ] . '\') {this.value = \'\'}" />';
		// End
					$showButton       = true;
					}
				else
		...
		if ( in_array( "availability", $searchOptions ) )
			{
                if($calledByModule == "mod_jomsearch_m3"){
                    ...
                    //add in searched region
                    $currRegion        = jomresGetParam( $_REQUEST, 'region_promo', "" );
                    if(isset($currRegion) && $currRegion != ""){
                        $output[ 'REGION' ] = "<input type=\"text\" style=\"display:none;\" value=\"".$currRegion."\" id=\"region_promo\" name=\"region_promo\">";
                        $output[ 'NODATES' ] = "<input type=\"text\" style=\"display:none;\" value=\"1\" id=\"nodates\" name=\"nodates\">";
                        $output[ 'MAPLIST' ] = "<input type=\"text\" style=\"display:none;\" value=\"maplist\" id=\"task\" name=\"task\">";
                    }
		...
		if ( $showButton == true )
			...
			if ( !using_bootstrap() ) $output[ 'THEBUTTON' ] = '<input type="submit" name="send" value="' . jr_gettext( '_JOMRES_SEARCH_BUTTON', _JOMRES_SEARCH_BUTTON, false ) . '" class="button" />';
                     ...
                     $output[ 'THEBUTTON' ] = '<button type="submit" class="btn btn-primary search-btn"><i class="icon-search"></i> ' . jr_gettext( '_JOMRES_SEARCH_BUTTON', _JOMRES_SEARCH_BUTTON, false ) . '</button>';
