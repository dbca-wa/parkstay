...
class j02260editbooking
	...
	function editBooking_html( $contract_uid, $bookingData, $extraBillingData, $guestData, $roomBookingData, $roomInfo, $roomClass, $roomFeatures, $bookersUsername )
		...
		foreach ( $bookingData as $booking )
            ...
            //$booking_departure = strtotime(date("Y-m-d", strtotime($booking_departure)) . " +1 day");
            //print_r($booking_departure);
		...
		foreach ( $roomInfo as $rInfo )
			...
			$rmInfo[ ]                  = array ( "rInfo_name" => $rInfo_name, "rInfo_number" => $rInfo_number, "rInfo_room_floor" => $rInfo_room_floor, "rInfo_room_disabled_access" => $rInfo_room_disabled_access, "rInfo_max_people" => $rInfo_max_people, "rInfo_type" => $rInfo_type, "rInfo_tariff" => $rInfo_tariff );
			}
		...
		if ( $thisJRUser->userIsManager )
			...
			if ( !$popup )
				...
				if ( $bookingData[ 0 ]->bookedout != "1" && $bookingData[ 0 ]->cancelled != "1" )
					...
					add_menu_option( '&task=amendBooking&no_html=1&contractUid=' . $booking_contract_uid, null, $output[ 'HAMENDBOOKING' ], null, jr_gettext( "_JOMRES_COM_MR_EDITBOOKINGTITLE", _JOMRES_COM_MR_EDITBOOKINGTITLE,false ) );
					}
				if ( get_showtime( 'include_room_booking_functionality' ) )
					{
					if ( FALSE && $bookingData[ 0 ]->bookedout != "1" && $bookingData[ 0 ]->cancelled != "1" )
						{
						if ( !$bookedin )
							{
							...
							add_menu_option( '&task=bookGuestOut&dueDepart=' . $booking_contract_uid, null, jr_gettext( '_JOMRES_FRONT_MR_MENU_ADMIN_BOOKAGUESTIN', _JOMRES_FRONT_MR_MENU_ADMIN_BOOKAGUESTIN, $editable = false, $isLink = true ), null, jr_gettext( "_JOMRES_COM_MR_EDITBOOKINGTITLE", _JOMRES_COM_MR_EDITBOOKINGTITLE,false ) );
							}
						}
					elseif (  $bookingData[ 0 ]->bookedout != "1" && $bookingData[ 0 ]->cancelled != "1" )
						{
						if ( !$bookedin )
							{
							$output[ 'HBOOKGUESTIN' ] = jr_gettext( '_JOMRES_FRONT_MR_MENU_ADMIN_BOOKAGUESTIN', _JOMRES_FRONT_MR_MENU_ADMIN_BOOKAGUESTIN, $editable = false, $isLink = true );
				...
				if ( FALSE && $bookingData[ 0 ]->bookedout != "1" && $bookingData[ 0 ]->cancelled != "1" )
					...
					add_menu_option( "&task=addServiceToBill&contract_uid=$booking_contract_uid", null, jr_gettext( '_JOMRES_COM_ADDSERVICE_TITLE', _JOMRES_COM_ADDSERVICE_TITLE, $editable = false, $isLink = true ), null, jr_gettext( "_JOMRES_COM_MR_EDITBOOKINGTITLE", _JOMRES_COM_MR_EDITBOOKINGTITLE,false ) );
					}
				if ( $bookingData[ 0 ]->bookedout != "1" && $bookingData[ 0 ]->cancelled != "1" )
					{
				...
				if ( $invoice_number > 0 )
					...
					add_menu_option( '&task=view_invoice&id=' . $invoice_number, null, $output[ 'SHOWINVOICE' ], null, jr_gettext( "_JOMRES_COM_MR_EDITBOOKINGTITLE", _JOMRES_COM_MR_EDITBOOKINGTITLE,false ) );
					}
				...
				add_menu_option( "&task=cancelBooking&popup=1&contract_uid=$booking_contract_uid", null, jr_gettext( '_JOMRES_COM_MR_EB_GUEST_JOMRES_CANCELBOOKING', _JOMRES_COM_MR_EB_GUEST_JOMRES_CANCELBOOKING, $editable = false, $isLink = true ), null, jr_gettext( "_JOMRES_COM_MR_EDITBOOKINGTITLE", _JOMRES_COM_MR_EDITBOOKINGTITLE,false ) );
				}
