...
function getResponse_guesttype(typeid, value) {
    ...
    var numCampersSubmit = 0;
    jomresJquery.each(jomresJquery("select[id^='guesttype']"),function(index){
        classname = jomresJquery(this).attr('class');
        if(classname == "adult" || classname == "adult valid"){
            numCampersSubmit += numCampersSubmit + jomresJquery(this).val();
        }

        if(numCampersSubmit > 0){
            jomresJquery('#availability').prop("disabled", false);
        }
        else{
            jomresJquery('#availability').attr('disabled','disabled');
        }
    });
};
...
function getResponse_rooms(field, value) {
    ...
    jomresJquery('#continue-two').prop("disabled", false);
};
