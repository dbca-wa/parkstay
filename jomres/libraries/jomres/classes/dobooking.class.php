...
class dobooking
	...
	function dobooking()
		...
		if ( get_showtime( 'task' ) == "handlereq" || get_showtime( 'task' ) == "dobooking" ) $this->suppress_output = false;
		...
		if ( !empty( $this->rr ) ) $this->requestedRoom = explode( ",", $this->rr );
		...
		if ( isset( $mrConfig[ 'margin' ] ) && !empty( $mrConfig[ 'margin' ] ) ) $this->margin = $mrConfig[ 'margin' ];
		else
		$this->margin = "0.00";
		$this->tariffModel = $mrConfig[ 'newTariffModels' ];
		if ( $this->tariffModel != "1" && $this->tariffModel != "2" ) $this->tariffModel = "1";
		...
		if ( !isset( $mrConfig[ 'cfg_weekenddays' ] ) ) $mrConfig[ 'cfg_weekenddays' ] = "5,6";
	...
	function getAllBookings()
		...
		if ( !$amend_contract ) $query = "SELECT room_uid,date FROM #__jomres_room_bookings WHERE property_uid = '$this->property_uid' AND $gor ";
		else
		...
		if ( count( $this->allBookings ) > 0 ) ksort( $this->allBookings );
		}
	...
	function get_fullybooked_dates()
		...
		if ( !isset( $result[ 'plugin_manages_fully_booked_dates' ] ) )
			...
			foreach ( $this->allPropertyTariffs as $tariff )
				{
				if ( $tariff[ 'dayofweek' ] == 7 ) $all_tariffs_are_specific_days_of_week = false;
				}
			if ( $all_tariffs_are_specific_days_of_week )
				...
				for ( $i = 0; $i <= $count; $i++ )
					...
					if ( in_array( $day_of_week_of_date, $days_of_week_allowed ) ) unset( $dates[ $i ] );
	...
	function errorChecking()
		{
		if ( $this->cfg_errorChecking == "1" ) return true;
		else
		return false;
		}
		...
		if ( $this->jrConfig[ 'useJomresMessaging' ] == '1' )
			...
			if ( isset( $this->growlmessages[ 'messages' ] ) && count( $this->growlmessages[ 'messages' ] ) > 0 )
				...
				foreach ( $this->growlmessages[ 'messages' ] as $message )
					{
					if ( $message != "" ) $messages .= 'jomresJquery.jGrowl(\'' . $message . '\', { life: 20000,sticky:true });';
					}
				}
	...
	function setPopupMessage( $message )
		...
		if ( $_REQUEST[ 'field' ] != "property_uid_check" ) $this->growlmessages[ 'messages' ][ ] = $msg;
		}
	...
	function echo_populate_div( $message )
		{
		if ( !$this->suppress_output ) echo $message;
		}
	...
	function getSingleRoomPropertyStatus()
		{
		if ( $this->cfg_singleRoomProperty == "1" ) return true;
		else
		return false;
		}
	...
	function toggleRoomFilterId( $id )
		{
		if ( isset( $this->room_feature_filter[ $id ] ) ) unset( $this->room_feature_filter[ $id ] );
		else
	...
	function check_room_has_selected_room_feature( $room_id )
		...
		foreach ( $bang as $f )
			{
			if ( $f != "" ) $feature_ids[ ] = (int) $f;
			}
		if ( count( $feature_ids ) == 0 ) return false;
		$count = 0;
		...
		if ( $count == count( $this->room_feature_filter ) ) return true;
		else
		return false;
		}
	...
	function checkRoomFeatureOption( $freeRoomsArray )
		...
		foreach ( $freeRoomsArray as $room_id )
			{
			if ( $this->room_acceptable_according_to_room_filter( $room_id ) ) $tmpArr[ ] = $room_id;
			}
	...
	function makeCustomerTypes( $selectedProperty )
		...
		foreach ( $exList as $ct )
			...
			$customerTypes[ 'DROPDOWN' ]  = jomresHTML::integerSelectList( $default_start, $ct->maximum, 1, 'guesttype' . $ct->id, 'size="1" class="'.$class.'" onclick="" onchange="getResponse_guesttype(\'' . $ct->id . '\',this.value);"', $defNo, "0", $use_bootstrap_radios = false );
	...
	function extraAlreadySelected( $extra )
		{
		$currentExtras = explode( ",", $this->extras );
		if ( in_array( $extra, $currentExtras ) ) return true;
	...
	function makeArrivalDateOutput( $arrivalDate )
		{
		if ( $this->cfg_fixedArrivalDateYesNo == '1' && $this->cfg_fixedPeriodBookings == '1' ) $selectionMethod = $this->fixedDaysArrivaldateDropdown( $arrivalDate );
		else
	...
	function initDepartureDate()
		...
		if ( $this->cfg_fixedPeriodBookings == "1" )
			{
			if ( $this->cfg_fixedPeriodBookingsShortYesNo == "1" ) $interval = $this->cfg_fixedPeriodBookingsShortNumberOfDays;
			else
	...
	function checkDepartureDate( $departureDate )
		...
		*/
		...
		if ( !$amend_contract )
			...
			$mrConfig = $this->mrConfig;
			if ( $mrConfig[ 'wholeday_booking' ] == "1" ) $interval = $interval - 1;
	...
	function makeDepartureDateOutput( $departureDate )
		...
		if ( $this->cfg_fixedPeriodBookings == '0' )
			{
			if ( $this->cfg_showdepartureinput == "1" ) return $this->generateDateInput( "departureDate", $departureDate, "dd" );
			else
	...
	function dateRangeIncludesWeekends()
		...
		if ( !empty( $this->dateRangeString ) ) $dateRangeArray = explode( ",", $this->dateRangeString );
		else
		return false;
	...
	function get_rate_per_night_converted_to_output_period( $ratepernight )
		...
		switch ( $this->cfg_booking_form_daily_weekly_monthly )
		{
			case "D":
				...
				break;
			case "W":
				if ( $this->cfg_tariffmode == "1" && $this->cfg_tariffChargesStoredWeeklyYesNo == "1" ) $num_period = $ratepernight;
				else
	...
	*/
	...
	function setGuest_country( $value )
		{
		if ( !isset( $value ) || empty( $value ) ) $value = $this->cfg_defaultcountry;
		$this->country = $value;
		}
	...
	function getExistingCustomerData( $id )
		...
		if ( $bookerClass == "100" )
			...
			if ( count( $result ) > 0 )
				...
				if ( $this->checkEmail( $result[ 'email' ] ) ) $this->setGuest_email( $result[ 'email' ] );
	...
	function makeGuestData()
		...
		*/
		...
		if ( strlen( $guest_country ) == 0 || is_null( $guest_country ) || $guest_country == "null" ) $guest_country = $this->cfg_defaultcountry;
	...
	function bookingCountriesDropdown( $selectedCountry )
		{
		if ( !isset( $selectedCountry ) || empty( $selectedCountry ) ) $selectedCountry = $this->cfg_defaultcountry;
	...
	function checkRoomNotAlreadySelected( $rtArray )
		...
		if ( count( $this->requestedRoom ) > 0 )
			...
			if ( count( $rmidsArray ) > 0 ) $rmidsArray = array_unique( $rmidsArray );
			if ( in_array( $room, $rmidsArray ) ) return false;
			else
			return true;
			}
	...
	function checkTariffTalliesWithRoom( $rtArray )
		...
		*/
		...
		foreach ( $this->allPropertyTariffs as $tariff )
			{
			if ( $tariff[ 'rates_uid' ] == $rtArray[ 1 ] && $tariff[ 'roomclass_uid' ] == $room_class ) $ratesList[ ] = $tariff[ 'rates_uid' ];
			}
		if ( count( $ratesList ) == 0 ) return false;
		else
		return true;
		}
	...
	function removeRoomuidsAlreadyInThisBooking( $freeRoomsArray )
		...
		if ( count( $this->requestedRoom ) > 0 && count( $freeRoomsArray ) > 0 )
			...
			foreach ( $freeRoomsArray as $id )
				...
				if ( !in_array( $id, $tmpArray ) ) $ret_uids[ ] = $id;
				}
			}
		else
		$ret_uids = $freeRoomsArray;
		if ( empty( $ret_uids ) && !empty( $tmpArray ) ) $this->setErrorLog( "removeRoomuidsAlreadyInThisBooking::Rooms removed as already in booking" );
	...
	function findFreeRoomsInDateRange( $freeRoomsArray )
		...
		if ( count( $freeRoomsArray ) > 0 )
			...
			if ( empty( $tmpArray ) ) $this->setErrorLog( "findFreeRoomsInDateRange::No free rooms found in date range" );
			}
		else
	...
	function checkPeopleNumbers( $freeRoomsArray )
		...
		if ( count( $freeRoomsArray ) > 0 )
			...
			for ( $i = 0, $n = count( $freeRoomsArray ); $i < $n; $i++ )
				...
				if ( $i < ( count( $freeRoomsArray ) - 1 ) ) $query .= " OR room_uid = ";
				}
			$query .= ") ";
	...
	function filter_tariffs_staydays( $tariff )
		...
		if ( $mrConfig[ 'tariffmode' ] == "2" )
			...
			if ( $stayDays >= $mindays && $stayDays <= $maxdays ) $stayDaysValid = true;
			}
		else
			{
		...
		if ( !$stayDaysValid ) $this->setErrorLog( "filter_tariffs_staydays:: Stay days not valid for tariff uid " . $tariff->rates_uid );
	...
	function filter_tariffs_on_dates( $tariff, $unixArrivalDate, $unixDepartureDate )
		...
		if ( !$datesValid ) $this->setErrorLog( "filter_tariffs_on_dates:: Dates not valid for tariff uid " . $tariff->rates_uid );
	...
	function filter_tariffs_alreadyselectedcheck( $tariff )
		...
		if ( !isset( $tariff->minrooms_alreadyselected ) || $tariff->minrooms_alreadyselected == "" ) $minrooms_alreadyselected = 0;
		else
		$minrooms_alreadyselected = $tariff->minrooms_alreadyselected;
		if ( !isset( $tariff->maxrooms_alreadyselected ) || $tariff->maxrooms_alreadyselected == "" ) $maxrooms_alreadyselected = 100;
		else
		...
		if ( !$roomsAlreadySelectedTests ) $this->setErrorLog( "filter_tariffs_alreadyselectedcheck:: Tariff already selected tariff uid " . $tariff->rates_uid );
	...
	function filter_tariffs_peoplenumbercheck( $tariff )
		...
		if ( $totalInParty > 0 )
			{
			if ( $totalInParty >= $minpeople && $totalInParty <= $maxpeople ) $numberPeopleValid = true;
			}
		else
		$numberPeopleValid = true;
		if ( !$numberPeopleValid ) $this->setErrorLog( "filter_tariffs_peoplenumbercheck:: People numbers don't match for tariff uid " . $tariff->rates_uid );
	...
	function filter_tariffs_dowcheck( $tariff )
		...
		if ( $weekendonly == "1" && $dateRangeIsAllWeekends ) $dowCheck = true;
		else
			{
		...
		if ( !$dowCheck ) $this->setErrorLog( "filter_tariffs_dowcheck:: Day of week check failed for tariff uid " . $tariff->rates_uid );
 ...
 */
	...
	*/
	...
	function limitRoomsList( $roomAndTariffArray )
		{
		if ( $this->booker_class == "100" ) return $roomAndTariffArray;
		...
		foreach ( $tariffsArray as $key => $val )
			...
			for ( $i = 0; $i < $returnRoomsLimit; $i++ )
				{
				$roomuid = @$val[ $i ];
				if ( isset( $val[ $i ] ) ) $tmpArray[ ] = array ( $roomuid, $tariffuid );
				}
			}
...
//print_r($roomDeets);
	...
	function generate_room_type_dropdowns()
		...
		foreach ( $dropdown_output as $routput )
            ...
            if($routput[ 'number_of_rooms' ] < 10) {
			    $r[ 'NUMBER_OF_ROOMS' ]  = $routput[ 'number_of_rooms' ];
			    $r[ 'NUMBER_OF_ROOMS_PRE' ] = "<span class=\"label label-warning\">";
	...
	function isAlreadyASelectedOption( $roomTariffOutputId )
		...
		if ( !empty( $this->requestedRoom ) )
			...
			foreach ( $currentUidsArray as $room_tariff )
				{
				if ( $roomTariffOutputId == $room_tariff ) return true;
				}
	...
	function makeRoomOverlibdata( $roomUid, $tariffUid, $roomTariffOutputId, $roomTariffOutputText, $removing = false )
        ...
        $tariffStuff[ 'CAPTIONAR' ]            = $captionar;
	...
	function GetTariffDetails( $tariffUid )
		...
		if ( empty( $this->cfg_ratemultiplier ) ) $this->cfg_ratemultiplier = 1;
		else
			...
			foreach ( $rateList as $rate )
				...
				foreach ( $dateRangeArray as $date )
					...
					if ( count( $numberOfGuestTypes ) > 0 )
						{
						if ( $unixDay <= $unixValidToDate && $unixDay >= $unixValidFromDate && ( $stayDays >= $rate->mindays && $stayDays <= $rate->maxdays ) && ( $this->total_in_party >= $rate->minpeople && $this->total_in_party <= $rate->maxpeople ) ) $pass = true;
						}
					else
						{
						if ( $unixDay <= $unixValidToDate && $unixDay >= $unixValidFromDate && ( $stayDays >= $rate->mindays && $stayDays <= $rate->maxdays ) ) $pass = true;
						}
					if ( $pass )
						{
	...
	function monitorBookingStatus()
		...
		if ( !get_showtime( 'include_room_booking_functionality' ) )
			...
			if ( count( $this->third_party_extras ) > 0 )
				...
				foreach ( $this->third_party_extras as $tpe )
					{
					if ( count( $tpe ) > 0 ) $quantity = 1; // We don't care how many extras there are, so long as at least one has been selected.
		...
		if ( $this->getMonitoringNumberOfMessages() == 0 ) $this->ok_to_book = true;
		}


	function outputZeroPrices()
		{
		if ( $this->getGuestVariantCount() > 0 ) $this->echo_populate_div( '; populateDiv("totalinparty","' . $this->getTotalInParty() . '")' );

		$staydays   = $this->getStayDays();
		$num_period = $this->get_part_of_stay_period( $staydays );
		$this->echo_populate_div( '; populateDiv("staydays","' . $num_period . '")' );
		if ( $this->cfg_tariffChargesStoredWeeklyYesNo == "0" ) $this->echo_populate_div( '; populateDiv("roompernight","' . output_price( "0.00" ) . '")' );
		$this->echo_populate_div( '; populateDiv("roomtotal","' . output_price( "0.00" ) . '")' );
		if ( $this->cfg_showExtras ) $this->echo_populate_div( '; populateDiv("extrastotal","' . output_price( "0.00" ) . '")' );
		if ( $this->jrConfig[ 'show_tax_in_totals_summary' ] == "1" ) $this->echo_populate_div( '; populateDiv("taxtotal","' . output_price( "0.00" ) . '")' );
		if ( $this->cfg_singlePersonSuppliment == "1" ) $this->echo_populate_div( '; populateDiv("single_suppliment","' . output_price( "0.00" ) . '")' );
		$this->echo_populate_div( '; populateDiv("grandtotal","' . output_price( "0.00" ) . '")' );
		if ( $this->cfg_showDeposit == "1" ) $this->echo_populate_div( '; populateDiv("deposit","' . output_price( "0.00" ) . '")' );
	...
	function setMonitoring( $message )
		{
		if ( !isset( $message ) || empty( $message ) ) $message = "Empty message!";
	...
	function selectedRoomsCanHostTotalInParty()
		...
		if ( $roomTotalBeds < $this->total_in_party ) return false;
		else
		return true;
		}
	...
	function tariffsCanHostTotalInParty()
		...
		$max   = doSelectSql( $query, 1 );
		if ( $max < $this->total_in_party ) return false;
		else
		return true;
		}
	...
	function setVariantValues()
		{
		if ( !get_showtime( 'include_room_booking_functionality' ) ) return true;

		if ( $this->setGuestTypeVariantValues() ) return true;
		else
		return false;
		}
	...
	function makeNightlyRoomCharges()
		...
		foreach ( $this->requestedRoom as $r )
			...
			if ( $ignore_pppn == "0" ) $this->allRoomsAreIgnorePPPN = false;
			}
		if ( $this->cfg_perPersonPerNight == "1" && $this->allRoomsAreIgnorePPPN ) $this->setErrorLog( "makeNightlyRoomCharges::Property is configured to charge Per Person Per Night, however all rooms have IGNORE PPPN set to yes, so rooms will be calculated per room, not per person. " );
		else
			{
			if ( $this->cfg_perPersonPerNight == "1" ) $this->setErrorLog( "makeNightlyRoomCharges::Property is configured to charge Per Person Per Night. One or more rooms are not set to Ignore PPN. All rooms will be calculated as per person per night as it is impossible for Jomres to ascertain how many people will be in each room." );
			}
		if ( count( $result ) > 0 )
			{
			foreach ( $result as $r )
				...
				if ( $this->cfg_perPersonPerNight == "1" && $this->allRoomsAreIgnorePPPN ) $total = $val * count( $this->requestedRoom );
				else
					{
					if ( $this->cfg_perPersonPerNight == "1" ) $total += $val;
					else
						{
			...
			if ( $this->cfg_perPersonPerNight == "1" ) $this->room_total = ( $total * $this->stayDays );
			else
	...
	function calcTotals()
		...
		$this->room_total_inc_tax_2 = $this->room_total + $this->single_person_suppliment + $this->tax;
				...
				if ( !empty( $extra ) )
					...
					switch ( $model[ 'model' ] )
					{
						...
						case '7': // per person per days min days
							...
							if ( $this->stayDays < $mindays ) $days = $mindays;
							else
							$days = $this->stayDays;
							$calc                                                        = $days * $thisPrice;
	...
	function calcSinglePersonSuppliment()
		...
		foreach ( $this->room_allocations as $rm_id => $allocation )
			...
			if ( $allRoomsMaxPeople[ $rm_id ] > 1 && $allocation[ 'number_allocated' ] == 1 ) $this->room_allocations[ $rm_id ][ 'suppliment' ] = $suppliment;
			else
	...
	function calcDeposit()
		...
		if ( $mrConfig[ 'depositIsOneNight' ] == "1" )
			...
			if ( $mrConfig[ 'use_variable_deposits' ] == "1" && count( $datesTilBooking ) <= (int) $mrConfig[ 'variable_deposit_threashold' ] )
				...
				if ( $this->cfg_depAmount == "1" ) $depositValue = $this->contract_total;
				else
					{
					if ( $this->cfg_depositIsPercentage == "1" )
						{
						if ( $this->cfg_roundupDepositYesNo == "1" ) $depositValue = ceil( ( $totalBooking / 100 ) * $depositValue );
						else
						$depositValue = ( $totalBooking / 100 ) * $depositValue;
						}
					else
						{
						if ( $this->cfg_roundupDepositYesNo == "1" ) $depositValue = ceil( $depositValue );
						else
	...
	function getRoomAllocationOutput()
		...
		foreach ( $this->room_allocations as $key => $val )
			...
			if ( isset( $val[ 'price_per_night' ] ) ) $price = output_price( $this->get_gross_price( $val[ 'price_per_night' ] * $val[ 'number_allocated' ], $this->accommodation_tax_rate ) );
			else
			$price = "";
			$room_number = "";
			$room_name   = "";
			...
			if ( $this->cfg_booking_form_rooms_list_style == "2" )
				...
				{
				if ( $this->cfg_bookingform_roomlist_showroomno == "1" ) $room_number = $this->allPropertyRooms[ $rm_id ][ 'room_number' ];
				if ( $this->cfg_bookingform_roomlist_showroomname == "1" ) $room_name = $this->allPropertyRooms[ $rm_id ][ 'room_name' ];
				$this->setPopupMessage();
	...
	function setFlatRate()
		...
		$tariffs = array ();
		$rates   = array ();
		if ( !isset( $mrConfig[ 'wisepriceactive' ] ) || empty( $mrConfig[ 'wisepriceactive' ] ) ) $mrConfig[ 'wisepriceactive' ] = '0';
		if ( !isset( $mrConfig[ 'wisepricethreshold' ] ) || empty( $mrConfig[ 'wisepricethreshold' ] ) ) $mrConfig[ 'wisepricethreshold' ] = '60';
		...
		if ( count( $tariffs ) == 0 ) return false;
		$datesTilBooking = $this->findDateRangeForDates( $this->today, $this->arrivalDate );
		...
		foreach ( $tariffs as $t )
			...
			if ( count( $datesTilBooking ) <= $wisepricethreshold && $mrConfig[ 'wisepriceactive' ] == "1" )
				...
				$disc[ $roomType ] = array ( "roomrate" => $tmpRate, "discountedRate" => $rate, "roomType" => $roomType, "isDiscounted" => $isDiscounted, );
				}
			else
				{
		...
		if ( $this->cfg_tariffChargesStoredWeeklyYesNo == "1" ) $rpn = $rpn / 7;
	...
	function getDiscountedRoomrate( $roomrate, $percentagebooked, $roomType )
		...
		if ( !isset( $mrConfig[ 'wiseprice10discount' ] ) || empty( $mrConfig[ 'wiseprice10discount' ] ) ) $mrConfig[ 'wiseprice10discount' ] = '30';
		...
		if ( $percentagebooked <= 75 )
			...
			if ( $percentagebooked <= 50 && $percentagebooked > 25 ) $discountPercentage = (float) $mrConfig[ 'wiseprice50discount' ];
			if ( $percentagebooked <= 25 && $percentagebooked > 10 ) $discountPercentage = (float) $mrConfig[ 'wiseprice25discount' ];
			if ( $percentagebooked <= 10 ) $discountPercentage = (float) $mrConfig[ 'wiseprice10discount' ];
					...
					foreach ( $dateRangeArray as $date )
						...
						if ( $pass )
							...
							if ( count( $datesTilBooking ) <= $wisepricethreshold && $mrConfig[ 'wisepriceactive' ] == "1" )
								...
								$disc[ $roomType ] = array ( "roomrate" => $tmpRate, "discountedRate" => $r, "roomType" => $roomType, "isDiscounted" => $isDiscounted, );
								$tmpBookingHandler->updateBookingField( "wiseprice_discount", $disc );
								$tmpBookingHandler->saveBookingData();
								}
							else
	...
	function te_setAverageRate()
		...
		foreach ( $this->requestedRoom as $rt )
			...
			if ( $tarifftypeid != false )
				...
				if ( count( $datesTilBooking ) <= $wisepricethreshold && $mrConfig[ 'wisepriceactive' ] == "1" )
					...
					$disc[ $roomType ] = array ( "roomrate" => $old_room_rate, "discountedRate" => $r, "roomType" => $roomType, "isDiscounted" => $isDiscounted, );
					$basic_room_rate   = $r;
					$tmpBookingHandler->updateBookingField( "wiseprice_discount", $disc );
					$tmpBookingHandler->saveBookingData();
					}
		...
		if ( $this->cfg_perPersonPerNight == "1" ) $this->rate_pernight = $total / $total_number_of_guests;
		else
	...
	function get_gross_price( $price, $tax_rate )
		{
			...
			if ( $this->makeRatePerNight() )
				...
				if ( $this->setVariantValues() )
					...
					if ( $this->makeNightlyRoomCharges() )
						...
						$this->calcDeposit();
						if ( $this->cfg_singleRoomProperty == 0 ) $this->outputDiscounts();
						}
					else
