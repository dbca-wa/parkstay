...
class jomres_property_maker
	...
	function create_new_property()
		{
		if ( !$this->check_cms_user_id() ) return false;
	...
	function insert_room_property_type_setting()
		...
		if ( $this->property_details[ 'is_real_estate_listing' ] == 0 )
			{
			if ( $this->property_details[ 'mrpsrp' ] == "MRP" ) $singleRoomProperty = "0";
			else
			...
			if ( !doInsertSql( $query ) ) $this->errors[ ] = "Sql error when saving new room";
	...
	function insert_property()
		...
		if ( $this->property_uid > 0 ) return true;
