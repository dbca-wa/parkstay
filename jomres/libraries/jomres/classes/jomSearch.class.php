...
//print_r($jrConfig['randomsearchlimit']);
//die();
...
//print_r($_REQUEST['nodates']);
		...
		if ( in_array( "town", $this->searchOptions ) )
            ...
            $query          = "SELECT  property_name,property_region,property_country,property_town FROM #__jomres_propertys WHERE published = '1' order by ptype_id";
            // end
            //print_r($query );
	...
	function jomSearch_region()
		...
		if ( !empty( $this->filter[ 'region' ] ) && $property_ors )
			...
			if ( $filter != '%' )
                ...
                if(isset($this->filter[ 'region_promo' ])){
                    $query              = "SELECT propertys_uid FROM #__jomres_propertys WHERE property_regionpromo LIKE '" . $this->filter[ 'region' ] . " $property_ors AND published='1' ";
                }
                else{
...
// Nitin Start
		...
		if ($filter == "australia" || $filter == "Australia") { $filter = "AU";}
// End
		$this->makeOrs();
		$property_ors = $this->ors;
...
// Nitin Start	
$region_id                = find_region_id( $filter );
if($region_id > 0) {
	$query              = "SELECT propertys_uid,ptype_id FROM #__jomres_propertys WHERE metakeywords LIKE '%$filter%' or property_region LIKE '%$region_id%' or property_description LIKE '%$filter%' or property_country LIKE '%$filter%' or property_name LIKE '%$filter%' or property_street LIKE '%$filter%' or property_town LIKE '%$filter%' $property_ors AND published='1' order by ptype_id ";
    }
else
{    $query   = "SELECT propertys_uid,ptype_id FROM #__jomres_propertys WHERE  metakeywords LIKE '%$filter%' or  property_country LIKE '%$filter%' or property_description LIKE '%$filter%' or property_name LIKE '%$filter%' or property_street LIKE '%$filter%' or property_town LIKE '%$filter%' $property_ors AND published='1'  order by ptype_id";
}
  //  echo "<br>"; 
//print_r ($query);
//echo"Hello";
...
//echo "1111";
		if ( $arrivalDate != "" && $departureDate != "" )
			...
			$unixCurrentDate = mktime( 0, 0, 0, $date_elements[ 1 ], $date_elements[ 0 ], $date_elements[ 2 ] );
			$secondsInDay    = 86400;
			$currentUnixDay  = $unixCurrentDate;
			$currentDay      = $arrivalDate;

            $dateBookable = true;
            if ($unixCurrentDate >= strtotime('+180 day')){
                $dateBookable = false;
            }

            if(isset($_REQUEST['region_promo'])){
                $unixCurrentDateFormat = date("y-m-d",$unixCurrentDate);

                if($_REQUEST['region_promo'] == 3929){
                    $advanceDateFormat = date("y-m-d",strtotime('+2 day'));
                    if ($unixCurrentDateFormat < $advanceDateFormat){
                        $dateBookable = false;
                    }
                }
                elseif($_REQUEST['region_promo'] == 3930){
                    // can book any days
                }
                elseif($_REQUEST['region_promo'] == 3931){
                    $advanceDateFormat = date("y-m-d",strtotime('+1 day'));
                    if ($unixCurrentDateFormat < $advanceDateFormat){
                        $dateBookable = false;
                    }
                }
                elseif($_REQUEST['region_promo'] == 3932){
                    $advanceDateFormat = date("y-m-d",strtotime('+1 day'));
                    if ($unixCurrentDateFormat < $advanceDateFormat){
                        $dateBookable = false;
                    }
