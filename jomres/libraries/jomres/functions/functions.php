...
function find_region_id( $region )
	...
	foreach ( $jomres_regions->regions as $r )
		{
// Nitin Start 
// added the region search 
        //	if ( strcasecmp( jomres_cmsspecific_stringURLSafe( $r[ 'regionname' ] ), $region ) == 0 )
    $pos = strpos(jomres_cmsspecific_stringURLSafe( $r[ 'regionname' ] ), $region);
   // print_r ($pos);
// end
    if($pos !== false)
			{
	...
	if ( $plugin_will_provide_lowest_price )
		...
		if ( $mrConfig[ 'is_real_estate_listing' ] == 0 )
			...
			if ( isset( $pricesFromArray[ $property_uid ] ) )
				{
				if ( $mrConfig[ 'prices_inclusive' ] == "0" ) $price = output_price( $current_property_details->get_gross_accommodation_price( $pricesFromArray[ $property_uid ], $property_uid ) * $multiplier, "", true, true );
				else
...
if ( $thisJRUser->userIsManager ) return "migs";
...
/**
#
* This function will make only the check box for bookable only and d*
#
*/    
...
//die();
	...
	if($_REQUEST['view'] == 'featured') $output .= 'disabled: true,';
	else
...
//    print_r($contractsList); die();
	for ( $i = 0, $n = count( $contractsList ); $i < $n; $i++ )
        ...
        $r['CHILD'] = $row->Child;        
        $r['ROOM'] = $row->room;
        $r['CONCESSION'] = $row->Concession;
        $r['NOTE']= $row->note;
        $r['PHONE']= $row->tel_landline;

        if($r['PHONE'] == ""){
            $r['PHONE']= $row->tel_mobile;
        }
        ...
        if(!$deposit_paid){
            if($contract_total == $deposit_required) {
                $remainder = $deposit_required;

                $remainder   = output_price( $remainder, null, false, true );
            }
        }
        elseif($deposit_paid && $contract_total == $deposit_required) {
            $remainder = '$0.00';
        }
        elseif($deposit_paid) {
            $contract_total = $row->contract_total;
            ...
            $remainder = $contract_total - $deposit_required;
            $remainder   = output_price( $remainder, null, false, true );
        }
        ...
        if(is_array($lineitems) && count($lineitems) > 1){
            ...
            $remainder   = output_price( $grand_total_inc_tax+$amount_already_paid, null, false, true );
        }
...
function dateDiff( $interval, $first_date, $second_date )
	...
	$fd             = gregoriantojd( $first_date_ex[ 1 ], $first_date_ex[ 0 ], $first_date_ex[ 2 ]  );
	$sd             = gregoriantojd( $second_date_ex[ 1 ], $second_date_ex[ 0 ], $second_date_ex[ 2 ] );
	$days           = $sd - $fd;
...
#
 */
function dateDiffBlacBooking( $interval, $first_date, $second_date )
{
