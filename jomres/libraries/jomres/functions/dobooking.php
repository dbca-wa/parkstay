...
if ( $userIsManager && in_array( intval( $_REQUEST[ 'selectedProperty' ] ), $thisJRUser->authorisedProperties ) && (int) $_REQUEST[ 'selectedProperty' ] > 0 && $thisJRUser->currentproperty != (int) $_REQUEST[ 'selectedProperty' ] )
    ...
    // Nitin Start
    $fax= $_REQUEST['fax'];
    //jomresRedirect( jomresURL( JOMRES_SITEPAGE_URL . "task=dobooking&selectedProperty=$selectedProperty" ), "" ); // org
    jomresRedirect( jomresURL( JOMRES_SITEPAGE_URL . "task=dobooking&selectedProperty=$selectedProperty&fax=$fax" ), "" );
    // End
	}
...
//print_r($tmpBookingHandler->tmpbooking);
...
//echo(rand() . "<br>");
//echo(rand() . "<br>");
$rdata =  date("Ymd") ."-" . mt_rand(100,1000) ;

			$query = "SELECT property_fax FROM #__jomres_propertys WHERE propertys_uid = '".$selectedProperty."'";
			$property_fax = doSelectSql($query,1);
			$fielddata['DEFAULT_VALUE'] = $property_fax . "/" . $rdata;
            //. date("ymdgi");
//echo "<br />.....RESULT of FIELDDATA........<br />";
  //          print_r($fielddata['DEFAULT_VALUE']);
...
// Nitin for tackling Purunulu kurrajong and walardi

	if($output[ 'PROPERTYUID' ] == 54 || $output[ 'PROPERTYUID' ] == 56)
	 $tmpl->addVar( 'cond', 'pid', 'avoid' );
	else
	 $tmpl->addVar( 'cond', 'pid', 'show' );
